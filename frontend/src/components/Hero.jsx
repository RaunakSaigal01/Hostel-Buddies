...
export default function Hero({ selectedBlock, onBack }) {
  ...
  // helper to get correct image from backend
  const getImageUrl = (fileName) => {
    const found = images.find((img) => img.file_name.toLowerCase() === fileName.toLowerCase());
  ...
  const fetchAvailability = async () => {
    try {
      const res = await fetch("http://127.0.0.1:5000/check-availability", {
        method: "POST",
  ...
  if (selectedBlock) {
    console.log("Looking for:", `${selectedBlock.letter}INFO.jpg`);
    ...
    <div className="code-window">
      ...
      {selectedBlock ? (
        <div className="code-content">
          <img
            src={getImageUrl(`${selectedBlock.letter}INFO.jpg`)}
        ...
        <div className="chatbot">
          ...
          *Don't take risk if chances are below 55%
          {/* Results */}
